...
2. Пользователь нажимает кнопку "Запустить аукцион" - аукцион переходит в статус ACTIVE. К аукциону подключаются 3 бота которые делают ставки, стараясь не перебивать ставки реальных игроков.
...
6. Anti-sniping (опционально при создании):
   - **Порог**: секунды до конца раунда, в течение которых ставки триггерят продление
   - **Продление**: секунды, на которые продлевается раунд
   - **Макс. продлений**: лимит продлений раунда
...
Веб-сайт:
Документация Swagger (Rest, socket.io):
Адрес сервера: